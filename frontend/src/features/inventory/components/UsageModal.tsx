...
import { useMaterials, useUsage } from '@/hooks/useMaterials';
...
export function UsageModal({ onClose, onSuccess, preselectedMaterialId, preselectedProjectId }: UsageModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (selectedMaterial && formData.quantity > selectedMaterial.quantity) {
      alert(`Không đủ tồn kho! Còn ${selectedMaterial.quantity} ${selectedMaterial.unit}`);
      return;
    }
    await usage.mutateAsync(formData);
    onSuccess();
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="bg-bg-secondary rounded-xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center p-4 border-b border-border">
          <h2 className="text-lg font-semibold text-text-primary">Xuất kho</h2>
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-4">
          ...
          <div>
            ...
            <div className="text-xs text-text-muted mt-1">
              Tồn kho: {selectedMaterial?.quantity || 0} {selectedMaterial?.unit || ''}
            </div>
          </div>
          ...
          <div className="flex gap-3 pt-4">
            ...
            >
              {usage.isPending ? 'Đang xử lý...' : 'Xuất kho'}
            </button>
          </div>
        </form>
      </div>
